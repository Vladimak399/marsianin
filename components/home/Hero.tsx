...
import { LocationId, getLocationLabel } from '@/data/locations';
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex min-h-svh w-full max-w-[1240px] flex-col px-4 pb-5 pt-5 sm:px-6 sm:pt-6 lg:px-12">
        <header className="flex items-start justify-between gap-3 text-[10px] tracking-[0.12em] text-[#808080]">
          <div>
            <p className="text-[clamp(1.55rem,8vw,2.2rem)] leading-none tracking-[0.03em] text-[#202020]">марсианин</p>
            <p className="mt-1 text-[10px] tracking-[0.1em] text-[#9c9c9c]">навигация по точкам</p>
          </div>
          <button
            type="button"
            onClick={() => router.push(`/menu?location=${selectedLocation ?? 'o12'}&category=завтраки`)}
            className="min-h-10 border border-[#d8d2cb] px-3 py-2 text-[11px] tracking-[0.08em] text-[#6f6f6f] transition-colors hover:bg-[#ffffffa6]"
          >
            меню
          </button>
        </header>
        ...
        <div className="relative mt-6 flex-1 lg:mt-8">
          <div className="relative z-20 max-w-[580px] space-y-3 sm:space-y-4">
            <p className="text-[10px] tracking-[0.1em] text-[#777777]">сетка · навигация · мобильный ввод</p>
            ...
            <p className="max-w-[520px] text-[clamp(0.9rem,3.6vw,1.08rem)] leading-relaxed text-[#666666]">
              выберите узел и сразу переходите к меню своей точки.
            </p>
          </div>
          ...
          <div className="relative hidden h-full min-h-[420px] lg:block">
            ...
            {nodes.map((node) => {
              ...
              return (
                <motion.button
                  ...
                  transition={{ duration: 0.18, ease: premiumEase }}
                >
                  <div className="mb-2 text-[10px] tracking-[0.1em] text-[#8d8d8d]">{node.technical}</div>
                  <div className="text-[clamp(2.8rem,5vw,4.8rem)] font-medium leading-[0.88] tracking-[0.02em] text-[#1d1d1d]">
                    {node.label}
                  </div>
                  <div className="mt-1.5 text-[10px] tracking-[0.12em] text-[#7a7a7a]">{node.coords}</div>
                  <div className="mt-1.5 flex items-center gap-3 text-[10px] tracking-[0.1em]">
            ...
            <div className="pointer-events-none absolute inset-x-0 top-0 flex justify-between text-[9px] tracking-[0.14em] text-[#9b9b9b]">
              <span>y: 30</span>
              <span>y: 20</span>
              <span>y: 10</span>
              <span>y: 00</span>
            </div>
            <div className="pointer-events-none absolute bottom-0 left-0 right-0 flex justify-between text-[9px] tracking-[0.14em] text-[#9b9b9b]">
              <span>x: 00</span>
              <span>x: 08</span>
              <span>x: 16</span>
              <span>x: 24</span>
        ...
        <footer className="mt-5 grid grid-cols-1 gap-2 border-t border-[#ddd7d1] pt-3 text-[10px] tracking-[0.08em] text-[#8d8d8d] sm:grid-cols-2 lg:grid-cols-4">
          <span>статус системы · точная синхронизация</span>
          <span>текущий маршрут · {getLocationLabel(selectedLocation ?? 'o12')}</span>
          <span>активные точки · o12 k10 п7</span>
