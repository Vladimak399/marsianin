...
import { LOCATIONS, LocationPoint, NearestLocation, Phase } from './types';
...
export default function DesktopScene({
  ...
  return (
    <div className="relative mx-auto hidden min-h-[100dvh] w-full max-w-[1180px] overflow-hidden bg-[#fffdf8] shadow-[0_16px_56px_rgba(0,0,0,.06)] lg:block">
      ...
      <div className="relative z-10 grid min-h-[100dvh] grid-cols-[390px_1fr] gap-10 px-10 pb-10 pt-32">
          ...
          <div className="mt-5 grid grid-cols-3 gap-2">
            {LOCATIONS.map((point) => (
              <button
                ...
                disabled={isBusy}
                className={`border border-black/[0.065] bg-white/72 px-3 py-3 text-left transition ${isBusy ? 'cursor-progress opacity-70' : 'hover:border-[#ed6a32]/45'}`}
              >
        ...
        <div className="relative overflow-hidden border border-[#ed6a32]/22 bg-white/58">
          <motion.svg aria-hidden className="pointer-events-none absolute inset-0 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none" initial={{ opacity: 0 }} animate={{ opacity: phase === 'open' ? 0.18 : 1 }} transition={{ duration: 0.32, ease: premiumEase }}>
            <motion.path d="M 16 68 C 30 38, 45 20, 59 29 S 79 52, 83 70" fill="none" stroke="#ed6a32" strokeOpacity="0.38" strokeWidth="0.7" strokeLinecap="round" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 0.9, ease: premiumEase }} />
            <motion.path d="M 18 24 C 36 46, 52 64, 86 84" fill="none" stroke="#ed6a32" strokeDasharray="1.4 2.3" strokeOpacity="0.22" strokeWidth="0.35" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 1.1, delay: 0.08, ease: premiumEase }} />
          </motion.svg>
          ...
          {desktopPoints.map((visual, index) => {
            ...
            return (
              <motion.button
                ...
                disabled={isBusy}
                className={`absolute z-20 w-[230px] -translate-x-1/2 -translate-y-1/2 border bg-white/88 p-4 text-left transition-opacity ${isBusy ? 'cursor-progress opacity-80' : ''}`}
          ...
          <AnimatePresence>{phase === 'open' && selected ? <LocationOpenPanel selected={selected} isBusy={isBusy} onBack={onBack} onSwitch={onSwitch} onOpenCategory={onOpenCategory} /> : null}</AnimatePresence>
        </div>
      </div>
    </div>
  );
